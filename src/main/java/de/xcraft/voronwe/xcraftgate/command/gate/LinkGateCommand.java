...
public class LinkGateCommand extends GateSubcommand {
   public LinkGateCommand(XcraftGate plugin) {
      super(plugin);
   }
   ...
   public void execute(String gateName, List<String> args) {
      ...
      if (gateName != null && gateTarget != null) {
         if (!this.gateExists(gateName)) {
            ...
            this.getGate(gateName).linkTo(gateTarget);
            this.reply("Linked Gate " + gateName + " to " + gateTarget);
         }
      } else {
         this.error("No gate(s) given.");
         this.reply("Usage: /gate link <gatename> <target_gatename>");
      }
